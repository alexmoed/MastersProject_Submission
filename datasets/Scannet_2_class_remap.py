"""
ScanNet20 ScanNet Data Efficient Dataset
...
class ScanNetDataset(DefaultDataset):
    ...
    def get_data(self, idx):
        ...
        if "segment20" in data_dict.keys():
            ...
            # ADDED: Remap to 3 classes - wall(0), floor(1), everything else(2)
            new_segment = np.full_like(data_dict["segment"], 2)  # Default to class 2 (other)
            new_segment[data_dict["segment"] == 0] = 0  # wall stays 0
            new_segment[data_dict["segment"] == 1] = 1  # floor stays 1
