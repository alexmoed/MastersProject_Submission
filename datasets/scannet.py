...
class ScanNetDataset(DefaultDataset):
    ...
    def get_data(self, idx):
        ...
        # Check for segment3 first (for 3-class dataset)
        if "segment3" in data_dict.keys():
            data_dict["segment"] = (
                data_dict.pop("segment3").reshape([-1]).astype(np.int32)
            )
        elif "segment20" in data_dict.keys():
            data_dict["segment"] = (
...
@DATASETS.register_module()
class ScanNet3Dataset(ScanNetDataset):
    VALID_ASSETS = [
        "coord",
        "color",
        "normal",
    ...
    class2id = np.array([0, 1, 2])  # 3 classes: wall, floor, other